...
# 6. 🌌 Numa 스타일 연속 조립형 시네마틱 스크롤리텔링 (Frontend)

Numa(`numa.uprock.pro`)와 Awwwards 스타일의 인터랙션을 결합한 60fps 연속 모핑 & 자석 결합(Magnetic Assembly) 룩북 랜딩 엔진입니다.

### 주요 기능

* **시네마틱 웜업 프리로더**: 750ms 동안 GPU 가속 및 폰트를 사전 예열하여 휠 조작 첫 프레임부터 버벅임(Jank) 0% 보장.
* **무(無)에서 유(有)로의 실시간 UI 프레임 조립**: 첫 화면의 극도의 순수 여백(Kinfolk Void)에서 출발하여 스크롤에 따라 상단 헤더와 하단 바가 눈앞에서 도킹.
* **사방 외곽 자석 캡슐 클라우드 결합 (Magnetic Assembly)**: 6대 실시간 데이터 캡슐(1초 틱, 1분봉 캔들, 10호가창, Gemini 84% 감성, 환율, 코레일 스나이퍼)이 3D 공간에서 회전하며 중앙으로 결합.
* **수평 키네틱 타이포 & Gemini AI 3줄 브리핑 순차 도킹**: 스크롤 속도와 1:1로 맞물리는 수평 유영 타이포와 수평 트랙 카드 스택.
* **타임라인 룰러 & 실시간 시계열 SVG 파형 모핑**: 스크롤 위상에 따라 국토부/코레일 데이터 파동 곡선이 유기적으로 형태 변형(Morphing).
* **2.7배 확장된 스크롤 트래블 & 페이즈별 안정적 홀드(HOLD Zone)**: 각 페이즈의 도킹 상태를 여유롭게 감상할 수 있는 호흡 구간 제공.
